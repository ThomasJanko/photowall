...
import { Camera, LayoutGrid, Loader2, Mail, RotateCcw } from "lucide-react";
  ...
  return (
    <PseudoGate>
      <main className="event-gradient-bg relative flex min-h-dvh flex-col items-center overflow-hidden px-5 py-8 sm:py-12">
        ...
        <div className="relative flex w-full max-w-md flex-1 flex-col items-center justify-center gap-6 md:max-w-lg">
          ...
          {previewUrl ? (
            <div className="w-full space-y-4">
              ...
              <div className="relative aspect-square w-full overflow-hidden rounded-3xl shadow-2xl ring-4 ring-white/20">
                <img
                  src={previewUrl}
                  alt="Aperçu"
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row">
                <button
                  type="button"
                  onClick={reset}
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-white/10 px-6 py-4 font-semibold text-white ring-1 ring-white/20 backdrop-blur-sm transition-transform active:scale-95"
                >
                ...
                <button
                  ...
                  className="flex-1 rounded-full bg-linear-to-r from-pink-500 to-purple-500 px-6 py-4 text-lg font-bold text-white shadow-xl shadow-pink-900/40 transition-transform active:scale-95 disabled:opacity-50 disabled:active:scale-100"
                >
                  {status === "uploading" ? (
            ...
            <label
              htmlFor="photo-input"
              className="inline-flex w-full max-w-xs cursor-pointer items-center justify-center gap-2 rounded-full bg-linear-to-r from-pink-500 to-purple-500 px-8 py-5 text-center text-lg font-bold text-white shadow-xl shadow-pink-900/40 transition-transform active:scale-95"
            >
              {status === "compressing" ? (
                <>
                  <Loader2
          ...
          <div
            className="min-h-6 w-full space-y-2 text-center"
            aria-live="polite"
          >
            {moderationEnabled && approvedNotice && (
              <p className="inline-block rounded-full bg-green-400/15 px-4 py-2 font-medium text-green-300 ring-1 ring-green-400/30">
                ✅ Ta photo est en ligne sur le mur !
              </p>
            )}
            {status === "success" && moderationEnabled && lastUploadPending && (
              <p className="inline-block rounded-full bg-amber-400/15 px-4 py-2 font-medium text-amber-200 ring-1 ring-amber-400/30">
        ...
        <Link
          href="/wall"
          className="relative mt-6 inline-flex items-center gap-2 rounded-full bg-white/5 px-6 py-3 font-semibold text-purple-200 ring-1 ring-white/20 backdrop-blur-sm transition-transform active:scale-95"
        >
        ...
        {config.features.privateMessages && (
          <Link
            href="/message"
            className="relative mt-3 inline-flex items-center gap-2 text-sm text-purple-300/80 transition-colors hover:text-purple-200"
          >
